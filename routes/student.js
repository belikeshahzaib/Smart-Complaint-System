...
router.post('/submit', [
  ...
  try {
    ...
    for (const admin of admins) {
      db.prepare('INSERT INTO notifications (user_id, complaint_id, message) VALUES (?, ?, ?)').run(
        admin.id, result.lastInsertRowid,
        `New complaint #${result.lastInsertRowid} submitted by ${req.session.fullName}: "${title}" [${priority.toUpperCase()}] at ${location}.`
      );
    }
